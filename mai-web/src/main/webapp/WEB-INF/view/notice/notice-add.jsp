...
<div class="row">
    ...
    <div class="col-md-3 col-md-offset-3">
     		...
     		<input id="J_obj_type" name="toPersonID" type="hidden"  value="${toPerson.personID }"/>
            <div class="modal-body" id="myModalBody">
                  <p class="text-center" >请输入通知内容</p>
                  <c:if test="${objType =='121'}"><!--活动通知  -->
                   <c:if test="${toPerson.personID!='' && toPerson.personID!=null}">
                   		<textarea class="form-control" rows="3" placeholder="发消息给手机：${toPerson.phoneNum}、姓名：${toPerson.name}的同学" rows="3" name="J_cont" id="J_cont"></textarea>
                   </c:if>
                   <c:if test="${toPerson.personID=='' || toPerson.personID==null}">
...
$(function(){
	...
	$("#J_draft").click(function(){
		var con = $("#J_cont").val();
		if(con){
		$("#J_draft").attr('disabled',"disabled");//禁止多次点击
			$.ajax({
	...
	$("#J_publish").click(function(){
		...
		var con = $("#J_cont").val();
		if(con){
		$("#J_publish").attr('disabled',"disabled");
			$.ajax({
