            ...
            <td id="operation_${actm.activitymemberID}">
              <c:choose>
                ...
                <c:when test="${actm.memberStatus==MEMBERSTATUS_CHECKKED}">
                  <button class="btn btn-default" type="button" data-toggle="modal" data-target="#myModal" onclick="auditmodal(this,'${actm.activitymemberID}','${actm.personID}',${actm.memberStatus},-1);">审核不通过</button>
              	  <!-- 发消息按钮 -->
              	  <button class="btn btn-default" id="J_msg" type="button" data-toggle="" data-target="" onclick="window.location.href='/notice/toAdd.action?objID=${actid}&objType=${NOTIFICATIONTYPE_TYPE_ACTIVITY_NEWS}&toPersonID=${actm.personID}'">发消息</button>
                </c:when>
      ...
      $.post($form.attr('action'), $form.serialize(), function(result) {
        ...
        if(result_json.errorcode == 0){
          ...
          if(result_json.member_status == ${MEMBERSTATUS_NORMAL}){
            ...
            $("#operation_"+result_json.amemberid).append("<button class=\"btn btn-default\"  type=\"button\" data-toggle=\"\" data-target=\"\" onclick=\"window.location.href='/notice/toAdd.action?objID=${actid}&objType=${NOTIFICATIONTYPE_TYPE_ACTIVITY_NEWS}&toPersonID="+result_json.personid+"'\">发消息</button>");
