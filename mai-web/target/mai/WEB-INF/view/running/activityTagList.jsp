...
<div class="jumbotron container-fluid">
    <div class="panel panel-default">
        <form action="/activityRunning/ActivityTagList.action" method="post" id="stform">
            ...
            <table class="table table-bordered table-hover table-responsive">
                ...
                <c:forEach  items="${tlist}" var="tag">
                    <tr id="${tag.tagID}">
                        <td id="${tag.tagID}_name"><c:out value="${tag.tagName}"></c:out></td>
                        <td><c:out value="${tag.tagCount}"></c:out></td>
                        <td>
                            <button class="btn btn-default" type="button" data-toggle="modal" data-target="#myModal" onclick="setValueModal(this,'${tag.tagID}',1);">编辑名称</button>&nbsp;
                            <button class="btn btn-default" type="button" onclick="window.location.href='/activityRunning/ActivityTagDetail.action?tid=${tag.tagID}'">查看详情</button>&nbsp;
                            <button class="btn btn-default" type="button" data-toggle="modal" data-target="#myModal" onclick="setValueModal(this,'${tag.tagID}',2);">删除</button>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form class="form-horizontal" action="/activityRunning/updateActivityTag.action" method="post" role="form" id="newvalue_form">
...
<script>
    $(function(){
        ...
        $("#delModalBtn").click(function(){
            ...
            var deltid = $("#tid").val();
            $.post("${ctxPath}/activityRunning/delActivityTag.action?tid="+deltid,function(data){
                if(data){
                    $('#myModal').modal('hide');
    ...
    function setValueModal(event,tid,_type){
        ...
        if(_type == 1){
            ...
            $("#newvalue_form").attr("action","/activityRunning/updateActivityTag.action");
            $("#delModalBtn").hide();
            $("#addModalBtn").hide();
            $("#saveModalBtn").hide();
            $("#myModalBtn").show();
            ...
            $("#newvalue_form").attr("action","/activityRunning/addActivityTag.action");
        }
        $("#tid").val(tid);
    }
